...
Instead of quantizing individual weight numbers into 4-bit or 8-bit grids, we represent entire $32 \times 32$ weight blocks as **discrete index pointers** ($K=64 \rightarrow$ 6 bits, $K=1024 \rightarrow$ 10 bits) into a shared dual codebook!
...
┌──────────────────────────────────────────────────────────────────────────┐
│  1. Store ONLY 6-bit / 10-bit block indices k and Codebooks (0.22 MB).    │
...
## 📊 Benchmark Results: TorchAO Native FP4 vs. K=64 & K=1024 Codebooks
...
| **Codebook K=64 (6-bit Index)** | 6-Bit Index / Block | `0.19 MB` | `0.58 bits` | `0.498454` | `14.6179` | `7.8737` | `0.571581` |
| **Codebook K=1024 (10-bit Index)** | **10-Bit Index / Block** | **`0.22 MB`** | **`0.97 bits`** | **`0.850093`** | `16.1059` | `14.9058` | **`0.886764`** |
...
- **Row 1 (Scatter Plots)**: Compares **Relative Magnitude Error (%) vs. Cosine Similarity** for TorchAO FP4 vs. $K=64$ & $K=1024$ Codebooks (Left) and Temperature Annealing Curve $\tau$ (Right).
...
- [`model.py`](model.py): Neural network architecture, `DualCodebookQuantizer` module, `CodebookRehydrationLinear` wrapper, and evaluation metrics.
- [`train_codebook.py`](train_codebook.py): FP32 reference model training, temperature-annealed codebook fine-tuning ($K=64$ and $K=1024$), and evaluation.
